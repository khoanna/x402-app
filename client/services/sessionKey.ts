...
import { http, parseAbi, parseUnits, type Hex } from "viem";
import { getSessionKey, storeSessionKey } from "@/utils/localStorage";
...
import { CallPolicyVersion, ParamCondition, toCallPolicy, toRateLimitPolicy } from "@botanary/permissions/policies";
...
export async function createSessionKey(): Promise<Hex> {
  ...
  storeSessionKey(sessionPrivateKey);
...
export async function approveSessionKey(signer: Signer, sessionKeyAddress: Hex, usdcPerPay: string): Promise<string> {
  ...
  const permissionPlugin = await toPermissionValidator(publicClient, {
    ...
    policies: [getTransferPolicy(usdcPerPay), rateLimitPolicy],
  });
...
export async function revokeSessionKey(signer: Signer, sessionKeyAddress: Hex, usdcPerPay: string): Promise<Hex> {
  const ecdsaValidator = await signerToEcdsaValidator(publicClient, {
    signer: signer,
    entryPoint: getEntryPoint("0.7"),
    kernelVersion: KERNEL_V3_3,
...
export async function usingSessionKey(approval: string) {
  const sessionPrivateKey = getSessionKey() as Hex;
  if (!sessionPrivateKey) {
    throw new Error("Please create and approve a session first.");
  }
  const sessionKeySigner = await toECDSASigner({
    signer: privateKeyToAccount(sessionPrivateKey),
  });
